...
func NewTokenClient(config *config.Config) *TokenClient {
	return &TokenClient{
		config: config,
...
func IsJWTTokenExpired(accessToken string) (bool, time.Time) {
	if token, err := jwt.Parse(accessToken, nil); token != nil {
		if err != nil {
...
func (a *TokenClient) GetToken() (*TokenResponse, error) {
	cachedTokenKey := fmt.Sprintf("%s%s", a.config.Endpoint, a.config.ClientID)
